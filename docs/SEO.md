# Medzae SEO & Discoverability Guide

Everything technical is already wired into the frontend. This document explains
what ships automatically and, more importantly, the **one-time manual steps**
that only the site owner can do — search engines will not know the site exists
until it is submitted to them once.

## What the code already does

| Surface | Where | What it serves |
|---|---|---|
| Titles, descriptions, keywords | `src/app/layout.tsx` + per-route `layout.tsx` | Keyword-rich `<title>`/`<meta>` on every page; auth-only pages are `noindex` so crawlers never index empty splash screens |
| Canonical URLs | root + route layouts | Point every indexable page at its one true URL |
| Open Graph / Twitter cards | `src/app/opengraph-image.tsx` | Branded 1200×630 share image for Google, WhatsApp, Slack, X, LinkedIn previews |
| Favicon / app icons | `src/app/icon.svg`, `apple-icon.tsx` | Browser tab, bookmarks, iOS home screen |
| `robots.txt` | `src/app/robots.ts` | Welcomes Googlebot/Bingbot **and every major AI crawler by name** (GPTBot, ClaudeBot, PerplexityBot, Grok, Google-Extended, Meta, Amazon, Apple…); hides auth-only routes |
| `sitemap.xml` | `src/app/sitemap.ts` | Lists `/`, `/signup`, `/login` for crawlers |
| Web app manifest | `src/app/manifest.ts` | PWA identity (name, colors, icons) |
| Structured data (JSON-LD) | root layout + landing page | `Organization`, `WebSite` (with alternate names like "medzae web"), `WebApplication`, and `FAQPage` — powers Google rich results and AI answer engines |
| `llms.txt` | `src/app/llms.txt/route.ts` | Plain-language site summary for AI assistants (ChatGPT, Claude, Perplexity) |

The site origin defaults to `https://medzae.com` — the owned domain, and the
canonical host — and is defined once in `src/lib/seo.ts`. Every surface above
derives its URLs from it, so changing the origin is a single change. The
`medzae.vercel.app` and `medihub-web.vercel.app` hosts stay attached and 308 to
it; see [The move to medzae.com](#the-move-to-medzaecom) for what that involved.

> **Never hardcode the origin anywhere else, and never point it at a host that
> redirects.** The rebrand shipped with the origin still set to the old
> `medihub-web` host, which meant the live site advertised canonicals,
> `og:url`, JSON-LD `@id`s, and a sitemap all pointing at a URL that only
> 307-redirects back — Google saw every page as a redirect to itself and had no
> stable URL to index. The same trap applies to `www`: the origin must match the
> primary Vercel domain exactly, including whether it carries the `www.` prefix.

## One-time manual steps (do these after deploying)

### 1. Google Search Console — covers Google + Chrome + most AI engines

The token baked into `src/app/layout.tsx`
(`2ujO5Tqw1xbRZZM--5YipXrF2LaxbqsANEmC9oQny4E`) belongs to the **old
`medihub-web` property**. A Search Console property is per-origin and each one
issues its own token, so the domain needs its own property.

Now that we control DNS for `medzae.com`, use a **Domain property** rather than
the HTML-tag method: one property covers `http`/`https`, `www`, and every
subdomain at once, and it never breaks on redeploy or on an env-var change —
whereas the meta tag verifies exactly one origin and silently unverifies if
`NEXT_PUBLIC_GOOGLE_SITE_VERIFICATION` is ever wrong.

1. In <https://search.google.com/search-console>, **Add property** → *Domain* →
   `medzae.com`.
2. Add the TXT record it gives you at the DNS host
   (NS1/NS2.HOSTING.BUSINESSIDENTITY.LLC), then click **Verify**. TTLs there
   are 1 minute, so it verifies almost immediately.
3. In Search Console → **Sitemaps**, submit `sitemap.xml`.
4. Leave `NEXT_PUBLIC_GOOGLE_SITE_VERIFICATION` unset — a Domain property does
   not use a meta tag, and the baked-in default is inert on this origin.
5. In **URL Inspection**, paste the homepage URL → **Request indexing**.
   Repeat for `/signup` and `/login`. Google usually indexes within hours to
   a few days.

Keep the old `medihub-web` property. It is where Google reports the old URLs,
and you need it to watch them drop out of the index as the redirect is
followed.

### 2. Bing Webmaster Tools — covers Bing, Edge, Yahoo, DuckDuckGo, ChatGPT search

1. Go to <https://www.bing.com/webmasters> and sign in.
2. Choose **Import from Google Search Console** (one click, reuses step 1) —
   or verify with a meta tag via `NEXT_PUBLIC_BING_SITE_VERIFICATION`.
3. Submit `sitemap.xml` there too, then use **URL Submission** for the
   homepage. Bing's index also feeds Yahoo, DuckDuckGo, Ecosia, and
   ChatGPT's web search.

### 3. Backlinks — the part that actually moves rankings

Search position is mostly decided by *who links to you*. Cheap, legitimate wins:

- Add the live URL to the GitHub repo's **About** field (Settings icon on the
  repo page → Website) and in `README.md`.
- Share the site on LinkedIn / X / Reddit (r/medicalschool etc.) — social
  links get crawled fast and seed AI training data.
- Ask any college/med-school pages, newsletters, or friends' sites to link
  to it.

### 4. Optional but high-impact later

- **Content**: a `/blog` or guides section targeting long-tail queries
  ("best note app for medical students", "how to keep up with medical
  news") is what eventually wins non-brand searches like "medical platform".

## The move to medzae.com

A `vercel.app` subdomain ranks noticeably worse than an owned domain, cannot be
verified as a Search Console *Domain* property, and shares its root domain's
reputation with every other Vercel project. "Medzae" is a coined word with
essentially no competing results, so the exact-match domain should take the
brand query quickly.

### DNS

DNS is authoritative at the registrar's host (NS1/NS2.HOSTING.BUSINESSIDENTITY.LLC),
**not** at Vercel — which is deliberate. The same zone carries the MX, SPF,
DKIM (`postal-kzzsgu._domainkey`), DMARC, and `psrp` records for the
`contact@medzae.com` mailbox. Moving nameservers to Vercel would drop them and
kill mail, so the domain is connected with records instead:

| Type  | Host  | Value                                    |
| ----- | ----- | ---------------------------------------- |
| A     | `@`   | `216.198.79.1`                           |
| CNAME | `www` | `5e5392b301ca8b74.vercel-dns-017.com.`   |

The `www` CNAME is a per-account hostname — read the current value from Vercel →
Domains → *View DNS configuration* rather than copying it from here. The
pre-existing wildcard `* → 66.223.49.89` is left in place; explicit records win
over it, so it only serves the registrar's parking page for stray subdomains.

**Never touch** the MX, `_dmarc`, `postal-kzzsgu._domainkey`, `psrp`, or SPF
records while editing this zone. Two things about mail are easy to get wrong
here and both fail silently:

- The domain publishes `p=quarantine` DMARC, so any mail claiming to be from
  `@medzae.com` that is not SPF- or DKIM-aligned goes to spam — including
  password resets. The app sends as the Gmail relay account, not as
  `@medzae.com`, so this is only a hazard if `SMTP_FROM_EMAIL` is ever pointed
  at a `medzae.com` address without authenticating that sender first.
- If you ever add a sending service, that means **editing the one existing SPF
  TXT record**, never adding a second. Two SPF records on a domain is a
  permanent error that fails SPF for everything, the real mailbox included.

### Remaining checklist

1. **`medzae.com` is the primary domain** in Vercel → Settings → Domains, with
   `www.medzae.com` redirecting to it, so only one host is canonical.
2. **`NEXT_PUBLIC_SITE_URL=https://medzae.com`** in Vercel's environment
   variables. It matches the `src/lib/seo.ts` default, so it is belt-and-braces
   rather than load-bearing — but it must never disagree with the primary
   domain.
3. **Keep both `vercel.app` hosts attached** and redirecting to the domain.
   Vercel's default project redirect is a **307 (temporary)** — switch each to
   **308 (permanent)** so Google transfers the ranking signals instead of
   holding both URLs. `medihub-web.vercel.app` currently chains through
   `medzae.vercel.app`; point it straight at `medzae.com` so there is one hop,
   not two.
4. **Add a Search Console *Domain* property** for `medzae.com` — see step 1 of
   the manual steps above.
5. **Submit the new `sitemap.xml`** and use **Change of Address** in the old
   property (Settings → Change of address) to tell Google the site moved.
6. **Update the non-SEO URLs that do not read `SITE_URL`**: the backend's
   `FRONTEND_URL` on Render (password-reset and welcome-email links point at
   the wrong host otherwise) and the Google OAuth client's **Authorized
   JavaScript origins** (see `docs/GOOGLE_SSO.md`) — sign-in breaks silently
   without the second one.

## How AI search finds Medzae

- **Crawling**: `robots.txt` explicitly allows GPTBot, OAI-SearchBot,
  ClaudeBot, PerplexityBot, Grok/xAI, Google-Extended, Applebot, Amazonbot,
  Meta, CCBot, and others.
- **Understanding**: JSON-LD (`WebSite` alternate names, `WebApplication`
  feature list, `FAQPage`) + `llms.txt` give engines clean, quotable facts.
- **Sourcing**: ChatGPT search rides on Bing, Perplexity/Claude fetch pages
  directly and both consult Google/Bing indexes — so steps 1–2 above are
  what put Medzae in AI answers. Expect AI engines to pick the site up
  days-to-weeks after classic search does.

## Verifying it works

- `https://medzae.com/robots.txt`, `/sitemap.xml`, `/llms.txt`,
  `/manifest.webmanifest`, `/opengraph-image` should all load — and every URL
  printed inside the first three must be on `medzae.com`, not a `vercel.app`
  host. `curl -s https://medzae.com/ | grep canonical` is the fastest check
  that the origin is right.
- `curl -sI https://medzae.vercel.app/` should answer **308** with
  `location: https://medzae.com/` — same for `www.medzae.com` and
  `medihub-web.vercel.app`. A 307 there means the redirect is still temporary
  and Google will keep both URLs instead of transferring the signal.
- Rich results test: <https://search.google.com/test/rich-results>
- Share-card preview: <https://www.opengraph.xyz> (or paste the link in
  WhatsApp/Slack).
- After a few days: search `site:medzae.com` on Google/Bing to confirm
  indexing.
